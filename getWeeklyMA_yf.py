...
def getWMA(sym='LT', period=20):
    ...
    data = yf.download(tickers=sym+'.NS', start=start, end=end, progress=False )
    #print(sym, start, end)
    #print(data)
    if data.empty:
        wma = 0.0
    ...
    wdata = data.drop(['Adj Close'], axis = 1)
    agg_dict = {'Open': 'first',
            'High': 'max',
            'Low': 'min',
            'Close': 'last',
...
def my_main():
    ...
    if (filename == 'getWeeklyMA_yf.py'):
        ...
        f = open("log_getWeeklyMA.txt", mode='w')    # open file in current directory
